...
/*  whileの構文 ------------------------------------------------------------------------------------------*/
    ...
    //while は「特定の条件が満たされている間は繰り返す」という状況で使われるのに対して
    //for は「一定の回数を繰り返す」という目的で使われることが多い
...
/*  continue. break ------------------------------------------------------------------------------------------*/
    ...
    for (let year = 1; year <= 30; year++) {
        amount *= 1.05;
        if (amount < 200) {   //表示するのは200からにしたい
            continue;
        }
        if (amount > 300) {   //300になったら抜ける
            break;
        }
...
/*  条件（三項）演算子の構文 ------------------------------------------------------------------------------------------*/
    ...
    const scores = Number(prompt('Score?'));
...
/*  論理演算子 ------------------------------------------------------------------------------------------*/
    ...
    const EnglishScore = Number(prompt('English?'));
    const MathScore = Number(prompt('Math?'))

    if (EnglishScore >= 80 && MathScore >=80) {
        console.log('A');
    } else {
        console.log('B');
    }
